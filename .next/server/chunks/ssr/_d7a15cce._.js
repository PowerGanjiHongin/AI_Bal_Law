...
async function findAvailableModels() {
    ...
    try {
        ...
        if (!response.ok) return `목록 가져오기 실패: ${data.error?.message}`;
        // 모델 이름들만 추출해서 합칩니다.
        const modelNames = data.models.map((m)=>m.name.replace("models/", "")).join(", ");
        return modelNames || "사용 가능한 모델이 없습니다.";
    } catch (err) {
        return `에러: ${err.message}`;
    }
}
function ResultPage() {
    const [models, setModels] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useState"])("목록 확인 중...");
...
//# sourceMappingURL=_d7a15cce._.js.map